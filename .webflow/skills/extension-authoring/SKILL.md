...
When such a page-scoped snapshot View describes one route (for example an SEO or per-page audit) and should let the user open that page, declare the `preview.openRoute` capability and open the page through it, passing only the route path (an absolute `/...` path) and letting Studio join it onto the project's loopback preview origin. Never emit a link or try to navigate the View itself — the host frame-navigation policy blocks that, and the View must not choose or handle the preview origin. The `extensions/view-starter` fixture shows the pattern: its `page.open` provided API delegates to `preview.openRoute`.
...
Choose `scaffold_composed_block_view_draft` before editing source when a new site-scoped View needs coordinated sections, a shared filter or input, master-detail behavior, several independently placed sections reacting to one small live value, or named installed supplier Blocks. A shared filter with installed supplier Blocks is supported authoring work. An ordinary single-surface View stays on `scaffold_extension_draft`; several presentational sections alone do not require Blocks. This scaffold creates an interim Extension-backed View, not the future project-owned View composition or fork format.
...
Supply logical state types, local Blocks, slots, ordered placement keys, per-placement configuration, coarse layout presets, and every explicit port-to-slot binding. Slots are View-owned: a Block port never creates a slot, so declare a shared slot once and bind every participating placement to it. Before placing a foreign Block, call `discover_admitted_extension_blocks` and copy only its exact Extension id, Block id, configuration schema version/value, publicly drivable input port, and compatible contract id/version. Never guess those values or inspect/copy supplier files, renderer source, APIs, actions, credentials, capabilities, package paths, trust records, or runtime identities. The host derives qualified local contract ids, exact foreign dependencies, versions, the View id, placement UUIDs, coordinates, renderer paths, entry markers, the manifest, composition, and generated declarations. Do not write or repair those structural files. Customize only the source and presentation paths returned by the tool, and validate both before and after customization. Re-run `scaffold_composed_block_view_draft` with the same draft id only for the exact original normalized specification. For a topology change, call `revise_composed_block_view_draft` with the existing draft id and the complete desired specification. If the original request is no longer in context and a version 2 host-owned `generated-scaffold.json` contains `specification`, read that value first; never edit that file. A legacy marker without it still requires the original complete specification. Revision preserves the draft, Extension, View, profile, and every still-owned customizable file; initialize only newly returned customizable files, then validate. If the View is already promoted, call `replace_extension_draft` only after the revised draft validates cleanly.
...
Semantic `provides` and `consumes` metadata supports discovery and compatibility only. It delivers no value, creates no connection, and grants no authority. View-local coordination instead requires separately declared typed Block state ports, View-owned slots, and explicit placement bindings; matching contracts never infer a binding. Across Extensions, only one write-only local Block owned by the hosting Extension may drive admitted foreign read ports that each declare `publiclyDrivable: true`, through a View-local slot with the exact shared contract and version. Foreign writers, multiple writers, peer-to-peer foreign bindings, inferred bindings, hosting reads of supplier state, and all cross-Extension APIs, actions, messages, and credentials remain refused. Use compare-and-set only for a value derived from a snapshot observed through a readable port. A write-only port cannot observe a revision: when it owns a complete replacement such as a shared filter input, call `publish(value, { replace: true })` and do not change its access to read-write just to obtain a revision. Any writable port may call `clear()` to commit its slot's valid unset state; readers and subscribers receive that state as an absent snapshot at a newer revision. Keep provider records and request state Block-local, and repair the named supplier, Block, port, slot, contract, configuration, or revision when diagnostics report an unavailable, undeclared, incompatible, unbound, invalid, oversized, stale-revision, or stale-runtime operation. The first-party `view-starter` may maintain its existing declared calls, actions, and explicit action bindings; project-scoped scaffolding does not author that topology, so do not invent actions there. Do not invent host channels, direct Selection access, capability handles, worker references, raw messages, credentials, or network access. Extension logic may use separately granted Studio capabilities while handling a same-Extension API call. Shared Context is an append-only, host-stamped provenance fact log, not mutable View state; Host Selection is separate and governed.
